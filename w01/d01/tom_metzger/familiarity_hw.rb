...
first_ans = 7 / 2 			# integer division returns integers rounded down

puts first_ans				# prints to console and includes line retrun

print first_ans				# prints to console but doesn't add line retrun

first_ans = 7 / 2.to_f		# makes it a float

first_ans = 7.to_f / 2 		# makes it a float

first_ans = 7 / 2.0 		# makes it a float

first_ans = 7.0 / 2 		# makes it a float

first_ans = first_ans.round * 4 	#rounds to 4 * 4 = 16



def get_character(full_string, index)		#defined function to pull a character?
    return full_string[index]
end


message_string = "oicdlcwhejkeenoemrstuo" 	#creates variable with string value



character_1 = get_character(message_string, 4) #pulls 5th letter "l"

character_2 = get_character(message_string, 7) #pulls 8th letter "h"

character_3 = get_character(message_string, 2) #pulls 3rd letter "c"



message_array = [character_1, character_2]		#creates array of ['l','h']

message_array.push(character_2)				#adds another 'h'

message_array.pop()							#removes last 'h'

message_array.push(character_3)				#adds 'c'
