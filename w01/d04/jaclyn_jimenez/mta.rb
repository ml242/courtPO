...
def number_of_stops(first_stop, final_stop)   #this method calculates number of stops for stops on the same line.
  ans = first_stop - final_stop
  ans.abs
end
...
if line_on == line_off          #if the station line names are the same
  ...
  first_ride = stops_for_multiple_transfers(mta[line_on], mta[line_off], first_stop)
  second_ride = stops_for_multiple_transfers(mta[line_off], mta[line_on], final_stop)
