...
while true
  ...
  when "display animals" then
    shelter.animals.values.each {|an| puts an.show_specs}
  when "display people" then
    shelter.people.values.each {|an| puts an.show_specs}
  when "adopt an animal" then
    adoptee = request_input("Which animal is being adopted?")
    adopter = request_input("Which person is adopting the animal?")
    shelter.adopt(adoptee, adopter)
  when "give animal up" then
    adopter = request_input("Who is giving a pet back to the shelter?")
    adoptee = request_input("Which pet are they giving up?")
    shelter.return_pet(adoptee, adopter)
